      ...
      <aside className={`fixed md:relative z-50 w-64 h-full bg-white dark:bg-slate-900 shadow-2xl md:shadow-none flex flex-col border-r border-slate-200 dark:border-slate-800 transition-transform duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
          <div className="flex flex-col">
            <h2 className="text-2xl font-black text-indigo-600 dark:text-indigo-400 font-heading tracking-tight">Maa Astha</h2>
            <p className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase mt-0.5">Admin Portal</p>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        <header className="bg-white dark:bg-slate-900 shadow-sm border-b border-slate-100 dark:border-slate-800 p-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button className="md:hidden p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" onClick={() => setIsSidebarOpen(true)}>
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700">
              ...
              <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{adminName}</span>
            </div>
            ...
            <button
              ...
              title="Toggle Theme"
            >
              {theme === "light" ? <CiDark size={22} strokeWidth={1} /> : <CiLight size={22} className="text-yellow-400" strokeWidth={1} />}
            </button>
