...
          {loading ? (
            <div className="p-10 text-center text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">Loading applications...</div>
          ) : filteredVolunteers.length === 0 ? (
            <div className="p-10 text-center text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">No {viewMode === "active" ? "pending" : "verified"} applications found.</div>
          ) : (
            filteredVolunteers.map((v) => (
              <div key={v._id} className="bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/50 p-5 rounded-2xl shadow-sm space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-bold text-slate-900 dark:text-white text-lg">{v.name}</div>
                    <div className="text-xs font-medium text-slate-500 mt-1">{v.gender}, {v.age} yrs • {v.profession || "N/A"}</div>
                  </div>
                </div>
                <div className="text-sm font-mono text-indigo-600 dark:text-indigo-400">{v.phone}</div>
                <div className="text-xs text-slate-500 border-b border-slate-100 dark:border-slate-700/50 pb-3">Applied: {new Date(v.createdAt).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric'})}</div>
                
                <div className="flex flex-col gap-2 pt-1">
                  <button onClick={() => setSelectedVol(v)} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-600 hover:text-white dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800/50 transition-all text-xs font-bold uppercase">
                    <Eye size={15} strokeWidth={2.5}/> Details
                  </button>
                  {v.status !== "Verified" ? (
                    <button onClick={() => handleVerify(v._id)} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-600 hover:text-white dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-800/50 transition-all text-xs font-bold uppercase">
                      <CheckCircle size={15} strokeWidth={2.5} /> Verify
                    </button>
                  ) : (
                    <span className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border bg-emerald-50/50 text-emerald-600 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20 text-xs font-bold uppercase">
                      <CheckCircle size={14}/> Verified
                    </span>
                  )}
                  {adminRole === "superadmin" && (
                    <button onClick={() => handleDelete(v._id)} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-red-50 text-red-600 border border-red-100 dark:bg-red-900/40 dark:text-red-400 text-xs font-bold uppercase mt-1">
